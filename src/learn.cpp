...
void learn(const Board &board, int rep_count)
{
   ...
   if (globals::theLog->current() == 0) return;
   ...
   if (globals::options.learning.position_learning && !last_entry.fromBook() && globals::theLog->current() >= 3) {
      ...
      if (out_of_book >= 3 && out_of_book <= 20) {
         ...
         if (globals::theLog->current()>=5) {
            const LogEntry &prev = (*globals::theLog)[globals::theLog->current()-5];
            if (!prev.fromBook()) {
#ifdef TUNE
         ...
         if (last_depth > globals::options.learning.position_learning_minDepth &&
            ...
            s << '\n';
            ofstream log;
            log.open(globals::learnFileName.c_str(),ios_base::out | ios_base::app);
            log << s.str();
            log.close();
            stringstream str;
