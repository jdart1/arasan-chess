...
enum StateType { NormalState, Terminated, Check, Checkmate, Stalemate, Draw, Resigns };
...
struct Statistics {
    ...
    uint64_t null_cuts;
    uint64_t razored;
    uint64_t check_extensions, capture_extensions, pawn_extensions, singular_extensions;
    uint64_t singular_searches;
    uint64_t multicut;
    uint64_t non_singular_reductions;
    uint64_t reduced;
    uint64_t lmp;
    ...
    struct MultiPVEntry {
        ...
        Move best;
        MultiPVEntry()
            : value(0), display_value(0), tb_value(0), fromBook(false), complete(false),
              best_line_image(""), depth(0), completedDepth(0), best(NullMove) {}

        MultiPVEntry(const Statistics &stats)
            : value(stats.value), display_value(stats.display_value), tb_value(stats.tb_value),
              fromBook(stats.fromBook), complete(stats.complete), best_line(stats.best_line),
              best_line_image(stats.best_line_image), depth(stats.depth),
              completedDepth(stats.completedDepth), best(stats.best_line[0]) {}
    };
    ...
    // reset counters but not pv, state, etc.
    void clearCounters();
